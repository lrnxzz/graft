...
func (b bot) Looking() (plugin.Target, bool) {
	hit, sighted := b.agent.Target(graft.BlockReach)
	if !sighted {
...
func (b bot) Inventory() []plugin.Stack {
	...
	for slot := range graft.InventorySize {
...
func (b bot) Hold(item plugin.Item) error {
	id, known := identified(item)
	if !known {
		return fmt.Errorf("graft: no item named %s", item)
	}
...
func (b bot) Look(at plugin.Vec3) {
	b.agent.LookAt(graft.Vec3(at.X, at.Y, at.Z))
}
...
func (b bot) Dig(at plugin.Vec3) error {
	...
	hit, err := b.agent.Dig(b.ctx, graft.BlockReach)
	if err != nil {
		return err
	}
	if hit.Block != target {
		return fmt.Errorf("graft: broke %v instead of %v", hit.Block, target)
	}
...
func (b bot) Place(at plugin.Vec3) error {
	...
	return b.agent.Place(graft.BlockReach)
}
...
func aim(goal plugin.Goal) (pathfinder.Goal, error) {
	...
	default:
		return nil, fmt.Errorf("graft: the %s goal is not served yet", goal.Type)
	}
}

func spot(at graft.Vec3d) plugin.Vec3 {
	return plugin.Vec3{
		X: at.X,
		Y: at.Y,
		Z: at.Z,
...
func block(at plugin.Vec3) graft.Position {
	return graft.At(int(at.X), int(at.Y), int(at.Z))
}

func named(id graft.ItemID) plugin.Item {
...
func identified(item plugin.Item) (graft.ItemID, bool) {
	known, is := items.Named(graft.Identifier(item))
	if !is {
		return 0, false
	}
